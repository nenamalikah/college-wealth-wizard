#%%
from RAG.components.preprocess import clean_cip_soc_code
import pandas as pd
...
for idx in crosswalk.index:
    xwalk.append(f'The associated SOC code for CIP code {crosswalk["CIP_Code"][idx]} is {crosswalk["SOC_Code"][idx]}. The associated field of study for a career in {crosswalk["SOC_Title"][idx]} is {crosswalk["CIP_Title"][idx]}')

crosswalk['XWalk_Summary'] = xwalk
print(f'The cip_soc_crosswalk shape is {crosswalk.shape}.')
print(f'Sample crosswalk text: {xwalk[100]}')
crosswalk.to_csv('../data/cip_soc_xwalk.csv',index=False) #  (6097, 5)
...
bls_text = []
for idx in bls.index:
    bls_text.append(f'In the {bls["NAICS_Industry_Title"][idx]} industry, the mean hourly wage for the {bls["SOC_Title"][idx]} occupation is {bls['Mean_Hourly_Wage'][idx]} and the mean annual wage is {bls['Mean_Annual_Wage'][idx]}. The total employment for the {bls["SOC_Title"][idx]} occupation in the {bls["NAICS_Industry_Title"][idx]} industry is {bls["Total_Employment"][idx]}. The SOC code for the {bls["SOC_Title"][idx]} occupation is {bls["SOC_Code"][idx]}.')

bls['Occupation_Summary'] = bls_text

print(f'BLS Shape {bls.shape}')
print(f'Sample BLS text: {bls_text[1000]}')
...
summary = []
for idx in df.index:
    summary.append(f'At {df["Institution_Name"][idx]}, here is the tuition information: {df["Tuition_Information"][idx]}. At {df["Institution_Name"][idx]}, here is the housing information: {df["Housing_Information"][idx]}. At {df["Institution_Name"][idx]}, here is the books information: {df["Books_Information"][idx]}. At {df["Institution_Name"][idx]}, here is the expenses information: {df["Expenses_Information"][idx]}. At {df["Institution_Name"][idx]}, here is the aid information: {df["Aid_Information"][idx]}. At {df["Institution_Name"][idx]}, here is the application fee information: {df["Application_Fee_Information"][idx]}. {df['Options_Summary'][idx]}')
