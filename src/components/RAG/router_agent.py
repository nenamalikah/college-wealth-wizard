...
# %%
def router_agent(repo_id, query):
    """
    ...
    router_template = PromptTemplate(
        ...
        If the question cannot be answered using one of the vector stores, route the question to your routing assistant. 
        
        Based on the question, return a dictionary with a single key 'datasource' and a list as the value containing one or more of the following choices: 'IPEDs vector store', 'BLS vector store', 'CIP_SOC vector store', or 'routing assistant'. Ensure that the output is in proper JSON format, with **double quotes** for keys and values. 
